...
function findShortestPath(){
    ...
    while(queue.length > 0){
        ...
        if(!(element.row + 1 >= maze.length)){
            addNeighbours({row: element.row + 1, col: element.col}, queue, visited, previousPosition, maze, {row: element.row, col: element.col});
            if(exitFound){
                break;
            }
        }
        ...
        if(!(element.col + 1 >= maze[0].length)){
            addNeighbours({row: element.row, col: element.col + 1}, queue, visited, previousPosition, maze, {row: element.row, col: element.col});
            if(exitFound){
                break;
            }
        }
    ...
    while(!(currentPosition.row == startPosition.row && currentPosition.col == startPosition.col) && exitFound){
        let currentRow = currentPosition.row;
        let currentCol = currentPosition.col;

        maze[previousPosition[currentRow][currentCol].row][previousPosition[currentRow][currentCol].col] = 't';
        steps++;
        currentPosition = {row: previousPosition[currentRow][currentCol].row, col: previousPosition[currentRow][currentCol].col};
    }

    function addNeighbours(position, queue, visited, previousPosition, maze, olderPosition){
        if(visited[position.row][position.col] == 0 && maze[position.row][position.col] != '*'){
    ...
    function findExits(){
        for(let i = 0; i < maze.length; i++){
            ...
            for(let j = 0; j < rowLength; j++){
                if((i == 0 || i == maze.length - 1 || j == 0 || j == rowLength - 1) && maze[i][j] == ' ' && maze[i][j] != 's'){
                    maze[i][j] = 'e';
                }
            }
        }
